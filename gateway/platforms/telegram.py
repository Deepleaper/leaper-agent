...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Commands are hardcoded here (not derived from COMMAND_REGISTRY) to
            # keep the Telegram hint menu minimal for a clean user experience.
            try:
                from telegram import BotCommand
                # Leaper: only register essential commands for clean user experience
                essential_commands = [
                    BotCommand("new", "开始新对话"),
                    BotCommand("help", "帮助"),
                ]
                await self._bot.set_my_commands(essential_commands)
                logger.info(
                    "[%s] Telegram menu: %d essential commands registered",
                    self.name, len(essential_commands),
                )
            except Exception as e:
                logger.warning(
