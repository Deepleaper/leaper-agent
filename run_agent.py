...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"LeaperAgent/{_HERMES_VERSION}",
    }
    ...
    def __init__(
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    client_kwargs["default_headers"] = {
                        ...
                        "X-OpenRouter-Title": "Leaper Agent",
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Leaper Agent.  Choose a model with at least "
        ...
        try:
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Leaper "
    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        if not (self._memory_manager and final_response and original_user_message):
            return
        try:
            logger.info("_sync_external_memory_for_turn: user_type=%s, resp_type=%s", type(original_user_message).__name__, type(final_response).__name__)
            self._memory_manager.sync_all(original_user_message, final_response)
            self._memory_manager.queue_prefetch_all(original_user_message)
        except Exception as _sync_exc:
            logger.warning("_sync_external_memory_for_turn failed: %s", _sync_exc)
