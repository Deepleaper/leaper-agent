...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    environment.
    """
    # Read once to avoid duplicate os.getenv calls below.
    ...
    if base_url_host_matches(custom_base, "openrouter.ai"):
        ...
        # configured. Check env vars before giving up — the user may have a real
        # custom endpoint configured via OPENAI_BASE_URL.
        if openai_base and not base_url_host_matches(openai_base, "openrouter.ai"):
            return openai_base, openai_key or "no-key-required", None
        return None, None, None
