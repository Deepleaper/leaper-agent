...
def _get_backend() -> str:
    ...
    # Final fallback: ddgs requires no API key, unlike firecrawl.
    # If ddgs is not installed the caller will fail with a clear ImportError.
    return "ddgs"
...
Create a markdown summary that captures all key information in a well-organized, scannable format. Include important quotes and code snippets in their original formatting. Focus on actionable information, specific details, and unique insights."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Use llm_output to avoid clobbering the `content` parameter above.
            llm_output = extract_content_or_reasoning(response)
            if llm_output:
                return llm_output
            ...
            if attempt < max_retries - 1:
                ...
                continue
            return llm_output  # Return whatever we got after exhausting retries
        except RuntimeError:
            logger.warning("No auxiliary model available for web content processing")
            return None
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if backend == "ddgs":
            ...
            raw_results = DDGS().text(query, max_results=min(limit, 10))
            web_results = []
